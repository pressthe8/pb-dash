...
export const processAllResultsForPRs = onCall<ProcessAllResultsForPRsRequest>(
  {
    // CRITICAL: Declare the secrets this function uses (for future compatibility)
    secrets: [concept2ClientId, concept2ClientSecret]
  },
  async (request): Promise<ProcessAllResultsForPRsResponse> => {
 ...
 */
async function processResultsForPRs(userId: string, results: Concept2Result[]): Promise<{ totalPRsProcessed: number; activitiesProcessed: number }> {
  try {
 ...
 */
async function assignScopesForActivity(userId: string, activityKey: string): Promise<void> {
  try {
    ...
    const events = activityEventsSnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    } as any));
    ...
    const eventsBySeason = events.reduce((acc: any, event: any) => {
